...
function LoginFormContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Jika berhasil, arahkan ke dashboard dan refresh state Next.js
      router.push("/admin/dashboard")
      router.refresh()
    }
  }

  // Menampilkan error dari URL (middleware) atau error lokal (salah password)
    ...
    <main className="min-h-screen w-full flex items-center justify-center bg-orange-50 p-4">
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-xs text-gray-400 mt-8 font-medium">
          &copy; {new Date().getFullYear()} Opsi Pintar. Portal Berbasis Keamanan.
        </p>
      </div>
    </main>
  )
